...
## Overview
Extend the canonical metadata builder to emit a deliberate descriptor `v3` shape instead of stretching either the original `v1` contract or the first-cut `v2` attempt. The `v3` descriptor should keep the existing release, command, assurance, canonicalization, and resolution surfaces, but replace ambiguous compatibility and versioning fields with clearer machine-readable contracts, keep explicit distribution-layout reporting, add explicit project-profile metadata, and retain a narrow semantic feature-token surface for high-level capability probing.

## Versioning Approach
- Bump to descriptor `v3` with one authoritative `schema_version` field rather than keeping redundant version fields with different types.
- Treat the `v3` descriptor as the only valid contract for the refined compatibility and feature semantics.
- Reject `v1`/`v2`/`v3` mixed payloads rather than attempting to support dual interpretation in one schema.
